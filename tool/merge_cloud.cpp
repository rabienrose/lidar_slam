	...
	//transfer point cloud into camera frame to map color
        ...
        if(point.at<double>(2)>0.2 && point.at<double>(2)<10)
        {
           ...
           imagepoint.y = tmpy * tmpdist
            ...
            if(50 <= px && px < imageSize_.width-50 && 50 <= py && py < imageSize_.height-50)
            {
                //int pid = py * w + px;
                pointTo.r = img.at<cv::Vec3b>(py,px)[0]*0.7;
                pointTo.g = img.at<cv::Vec3b>(py,px)[1]*0.7;
                pointTo.b = img.at<cv::Vec3b>(py,px)[2]*0.7;
                pointTo.a = 255;
...
int main(int argc, char** argv){
    ...
    std::string pathBag="/home/albert/Desktop/ParkingLot/2018-09-27-17-03-39_anticlockwise.bag";
    std::string traj_path="/home/albert/Desktop/ParkingLot/traj.txt";
    ...
    for (const rosbag::MessageInstance& m : view) {
        ...
        if (s1 != NULL && m.getTopic() == point_topic) {
            ...
            for (int i = 0; i < cloudSize; ++i)
            {
                ...
                point.x = temp_pointcloud->points[i].y;
                ...
                //temp_pointcloud->points[i] = point;
            }
...
//     downSizeFilter.filter(*global_cloud_ds);
    ...
    pcl::io::savePCDFile (root_save_folder+ "/point_cloud_merged_color.pcd", *global_cloud, true);
